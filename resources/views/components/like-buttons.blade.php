<div class="flex">
    <form method="POST" action="/tweets/{{$tweet->id}}/like">
        @csrf
        <button type="submit" class="text-xs flex mr-4 ">
            <div
                class="flex {{ $tweet->isLikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500'}} ">
    ...
    <form method="POST" action="/tweets/{{$tweet->id}}/like">
        ...
        <button type="submit" class="text-xs flex">
            <div class="flex {{ $tweet->isDislikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500'}}">
                <svg viewBox="0 0 20 20" class="mr-1 w-3">
                    <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                        <g class="fill-current">
                            <path
